...
| free_comment | TEXT | 可 | 自己評価に添えるコメント(任意) |
...
2テーブル同一構造。**大分類 → 課題(対策)の2階層**。

| カラム | 型 | NULL | 説明 |
|---|---|:-:|---|
| id | BIGINT UNSIGNED PK | ― | |
| parent_id | BIGINT UNSIGNED FK(自己参照) | 可 | NULL=大分類、値あり=課題(対策) |
| name | VARCHAR(100) | ― | 名称 |
| description | VARCHAR(500) | 可 | 説明。週報入力時に選択の目安として表示する |
| scope | ENUM('general','executive') | ― | 適用範囲。general=メンバー・所属長、executive=役員 |
| sort_order | INT DEFAULT 0 | ― | 表示順 |
| is_active | BOOLEAN DEFAULT TRUE | ― | 無効化で運用(削除しない) |
| created_at / updated_at | DATETIME | ― | |

- `description` は大分類・第2階層のどちらにも設定できる
- 週報入力画面では、選択中の項目の説明を表示する(第2階層が未選択なら大分類の説明)
- `scope` は大分類・第2階層で同じ値にする(子は親の適用範囲を引き継ぐ)。
  週報入力時は提出者のロールに対応する `scope` のみを選択肢として表示し、
  保存時にも適用範囲が一致するか検証する
